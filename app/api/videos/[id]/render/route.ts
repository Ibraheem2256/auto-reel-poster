...
/**
 * POST /api/videos/[id]/render — renders the video with the current edit spec
 * at preview quality (540x960) and streams it back as a video/mp4 download.
 * Progress is reported via Server-Sent Events and the binary payload follows
 * a sentinel marker, so one request gives progress + the file.
 */
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        ...
        // Sentinel, then the raw binary file.
